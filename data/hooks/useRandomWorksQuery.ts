...
const departmentsWithTopWorks = departments.map((department) => ({
  ...
  randomWork: data.data.filter((work: any) => department.department === work.department).slice(0, 8).sort(() => Math.random() - 0.5)[0]
}));

const randomDepartments = departmentsWithTopWorks.sort(() => Math.random() - 0.5)
